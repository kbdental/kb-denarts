...
| BI-002 | 2026-08-13 | Attendance / Inventory (backend) | P1 | Data Integrity | `backend/Code.gs` did a blind `clearContents()`+rewrite on every push. Any device pushing from a slightly-stale pull could silently erase records another device had just added — the more devices sync, the worse. Directly caused entries made on one device to intermittently vanish or never appear on others. | CR-003 | 9 targeted unit tests incl. a direct two-device race reproduction, all passing. **Requires manual Apps Script redeploy by the clinic — code fix alone does not take effect in production.** | Fixed in repo, **not yet live** — pending clinic redeploy |
